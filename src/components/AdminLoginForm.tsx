...
export default function AdminLoginForm({ onLoginSuccess }: { onLoginSuccess: () => void }) {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Check user role in public.profiles table (Must be superadmin, editor, or admin)
      ...
      if (!allowedRoles.includes(profile.role)) {
        await supabase.auth.signOut();
        throw new Error(`Access Denied: Your account role (${profile.role}) does not have Admin Portal access.`);
      }
  ...
  return (
    <div className="min-h-screen bg-[#FCFCF9] dark:bg-[#121212] flex items-center justify-center p-4 font-serif">
      <div className="max-w-md w-full border border-stone-300 dark:border-stone-800 bg-[#FCFCF9] dark:bg-[#161616] p-8 rounded-lg shadow-2xl space-y-6">
        <div className="text-center border-b border-stone-300 dark:border-stone-800 pb-4">
          ...
          <p className="text-xs font-mono text-stone-500 mt-1 uppercase">
            RESTRICTED ACCESS • SUPERADMIN &amp; EDITORS ONLY
          </p>
        </div>

        {error && (
          <div className="p-3 border border-red-300 dark:border-red-900 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300 text-xs font-mono rounded flex items-center gap-2">
        ...
        <form onSubmit={handleAdminLogin} className="space-y-4 font-mono text-xs">
          <div>
            <label className="block text-stone-600 dark:text-stone-400 mb-1">Admin Email Address:</label>
            <div className="relative">
              <Mail className="w-4 h-4 absolute left-2.5 top-2.5 text-stone-400" />
              <input
                type="email"
                required
                placeholder="admin@eevolution.edu"
                value={email}
          ...
          <div>
            <label className="block text-stone-600 dark:text-stone-400 mb-1">Admin Password:</label>
            <div className="relative">
          ...
          >
            ...
            <span>{loading ? 'Verifying Admin Roles...' : 'Authorize & Enter Admin Panel'}</span>
          </button>
        </form>

        <div className="pt-4 border-t border-stone-200 dark:border-stone-800 text-[11px] font-mono text-stone-500 text-center">
          <p>No public signups allowed on Admin Panel.</p>
          <p className="text-stone-400 mt-0.5">Contact system superadmin to grant editor credentials.</p>
        </div>
      </div>
    </div>
  );
